...
def main(): #new main function
    ...
    #chop = raw_input("Do you want the bottom half of the secondary spectra? (Y or N) ") 
    namefile = open("/Users/lelemathis/Dropbox/Dan Research/pulsarnames.txt", "r")#open file with list of pulsar names
    ...
    for i in range(0,len(psrNames)):
        hdulist = fits.open('/Users/lelemathis/Spectra/PSR'+psrNames[i]+'_dyn.fits')
        saveSpectra(hdulist,psrNames[i], cmap) #passes the data in as astrodata, pulsar name as psrName
 
def saveSpectra(hdulist, psrName, colorMap): #the main function of makespectra2
